...
async function updateDashboard() {
    ...
    const shiftBody =
        document.querySelector("#shiftTable tbody");
    ...
    const zBody =
        document.querySelector("#zernikeTable tbody");
    ...
    if(Object.keys(data.turbulence).length > 0){
        ...
        document.getElementById("r0").innerHTML =
            data.turbulence.r0 + " cm";

        document.getElementById("tau0").innerHTML =
            data.turbulence.tau0 + " ms";
    ...
    const dmTable =
        document.getElementById("dmTable");

    dmTable.innerHTML="";

    const values =
        Object.values(data.dm_commands);
        ...
        for(let c=0;c<8;c++){
            ...
            if(index<values.length){
                ...
                if(v>=0){
                    ...
                    `rgb(0,${255-intensity},255)`;

                }
                else{

                    td.style.backgroundColor=
                    `rgb(255,${255-intensity},0)`;
...
function drawWavefront(wavefront){
    ...
    const canvas =
        document.getElementById("wavefrontCanvas");
    ...
    for(let y=0;y<rows;y++){
        ...
        for(let x=0;x<cols;x++){
            ...
            const norm=
                (value-min)/(max-min+0.00001);

            const red=
                Math.floor(255*norm);

            const blue=
                Math.floor(255*(1-norm));
